...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    if (!items || items.length === 0) {
      return res.status(400).json({ message: 'Cần có ít nhất một sản phẩm để tạo đơn hàng.' });
    }
...
exports.getOrderDetails = async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ message: 'Lỗi hệ thống khi lấy chi tiết đơn hàng!' });
    }
  };
...
exports.deleteOrder = async (req, res) => {
    ...
    try {
      ...
      if (!order) {
        return res.status(404).json({ message: 'Đơn hàng không tìm thấy hoặc không phải của bạn.' });
      }
